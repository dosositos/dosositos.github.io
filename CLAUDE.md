...
**Las frases del juego van emparejadas en su forma de escribir.** Armando
escribe en minúscula, abrevia («m», «t», «q», «cn», «vdd») y se come tildes;
Jennifer empieza en mayúscula, escribe completo y tildea. Sin emparejar eso, «¿quién
dijo esto?» se resuelve con la primera letra. `scripts/preparar-juego.mjs` aplica
la tabla de `private/juego-normalizacion.json` (abreviaciones, ortografía,
interrogativas, nombres propios y correcciones frase por frase) y avisa si queda
alguna fuga. **Los dedazos, las risas y los alargues no se tocan nunca**: eso es
lo gracioso y está repartido entre los dos. La primera pantalla del juego lo
explica — normalizar callado sería trampa.
...
`frases-inventadas.json` o `juego-normalizacion.json` dispara antes
`preparar-juego.mjs`, que es quien escribe `private/publicable/juego.json`. Al tocar ese contenido, incluí
